...
author: |
    | \small Juan C. Rocha^1,2^, Kevin Berry^3^, Marianne Faladeau-Cote^4^, Anne-Sophie Crépin^5^, Susa Niiraren^1^
...
Two evolutionary features of resilience is adaptive and transformative capacities.
...
They transform their main economic activity into whale watching and today is a popular ecotourism destination [ARA].
...
We report the Z-transformed version of the comlexity index, values \> 0 represent locations or professions with higher complexity than the average for each dataset [Hidalgo].
...
We then hypothesise that a statistical relatonship between knowledge complexity and out migration rates can be detected in our dataset.
...
Finland's economic complexity is concentrated in the areas close its capital Helsinki.
